...
날짜를 저장할 수 있고, 날짜와 관련된 메서드도 제공해주는 내장 객체인  Date는 생성 및 수정 시간을 저장하거나 시간을 측정할 수 있고, 현재 날짜를 출력하는 용도 등으로도 활용할 수 있다.
...
- JS 데이터 ⬅️ JSON : `JSON.parse(data)` (역직렬화)
...
- replacer 콜백함수 사용
  ...
  콜백함수로 특정 프로퍼티일 때 `undefined` 를 반환하게 하면 해당 프로퍼티를 제외시켜서 직렬로 할수 있다.
...
```
 ...
 역직렬화 결과 `meetup.date` 의 값은 문자열이기 때문에 위와 같은 에러가 발생한다.
...
```js
let str = '{"title":"Conference","date":"2021-09-25T12:00:00.000Z"}';
...
reviver 메서드를 정의하여 키값이 `date` 일 때 해당 값을 Date객체로 만들도록 지정해서 온전한 값으로 되살릴수 있다.
